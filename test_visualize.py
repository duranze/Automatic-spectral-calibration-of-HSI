...
if __name__ == "__main__":
    ...
    for i, name in enumerate(load_net_name_list):
        ...
        test_dataset = dataset.HS_multiscale_DSet(opt,'test')
        ...
        test_nir_dict = {}
        test_whole_dice = {}
        dict_list = [test_whole_dice,test_vis_dict,test_nir_dict]
        recv_list = ['whole','vis','nir']
        for j,(hsi_sc, hsi_gt,img_name) in enumerate(test_loader):            # To device
            hsi_sc = hsi_sc.cuda()
            img_name = img_name[0]
            print(j, img_name)
            # imgid = img_name[:-4]
            ...
            for r in range(3):
                ...
                # if r==0:
                #     gen_recv_error_map(out_list[r],gt_list[r],image_gt,img_name,epoch_num,'./visual-test/'+recv_list[r]+'/')
        ...
        for m in range(3):
            ...
            with open('test_'+recv_list[m]+'_'+epoch_num+'.json', 'w') as json_file:
