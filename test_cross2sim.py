...
if __name__ == "__main__":
    ...
    method_name='sit'
    import os
    def write_result(res,method_name='sit',split='test'):
        model_dir = '/amax/home/dzr/dzr/dzr/autoCalib/Mine/visual-paper/hsi_real/cross2sim/'
        if not os.path.isdir(model_dir):
            os.makedirs(model_dir, exist_ok=True)
        save_name = method_name+'_'+split+".txt"
    ...
    load_net_name_list.append('./track1/G_epoch500_bs4.pth')
    import json
    for i, name in enumerate(load_net_name_list):
        ...
        demo_dataset = dataset_sim.HS_multiscale_DSet(opt,'test')
        ...
        val_txt = ''
        # _v: normal illumination; _o: filter illumination
        group_v = ['sd','cd','ev','rd','sh']
        group_o = ['bl','re','ye','pu','gr']
        ...
        for j,(hsi_sc, hsi_gt,img_name) in enumerate(demo_loader):            # To device
            ...
            if sc_name in group_v:
                psnr_v += calc_psnr(hsi_gt,out)
                sam_v += calc_sam(hsi_gt,out)
                rmse_v += calc_rmse(hsi_gt,out)
                ergas_v += calc_ergas(hsi_gt,out)
            else:
                psnr_o += calc_psnr(hsi_gt,out)
                sam_o+= calc_sam(hsi_gt,out)
                rmse_o += calc_rmse(hsi_gt,out) 
                ergas_o += calc_ergas(hsi_gt,out)
        ...
        ergas = ergas / length
        sam = sam / length
        psnr_v = psnr_v * 2 / length 
        rmse_v = rmse_v* 2 / length 
        ergas_v = ergas_v* 2 / length
        sam_v = sam_v* 2 / length
        psnr_o = psnr_o* 2 / length
        rmse_o = rmse_o* 2 / length
        ergas_o = ergas_o* 2 / length
        sam_o = sam_o* 2 / length
